              ...
              <Avatar
                src={
                  selectedUser && selectedUser.image
                    ? `${process.env.REACT_APP_BACKEND_URL}/${selectedUser.image}`
    ...
    return (
      <>
        <Grid container spacing={2} className={classes.mb3}>
          ...
          <Grid item xs={4}>
            <SettingsFormGroup label="Новости" values={permission['news']} handleChange={this.handleChange('news')} />
          </Grid>
          <Grid item xs={4}>
            <SettingsFormGroup label="Чат" values={permission['chat']} handleChange={this.handleChange('chat')} />
          </Grid>
        </Grid>
