...
const userProfile = handleActions(
  {
    [setProfileData]: (state, action) => {
      const { _id: id, image, middleName, surName, username, firstName } = action.payload;
      console.log('Auth data:');
      console.log(action.payload);
      const newImage = image ? `${process.env.REACT_APP_BACKEND_URL}/${image}` : image;
      return { id, image: newImage, firstName, middleName, surName, username };
    },
    [logoutUser]: () => null,
  },
  null
...
const tokens = handleActions(
  {
    [setTokenData]: (state, action) => {
      const { accessToken, accessTokenExpiredAt, refreshToken, refreshTokenExpiredAt } = action.payload;
      return {
        accessToken,
...
export const registerUser = ({ username, password, firstname, lastname, patronicname }) => (dispatch, getState) =>
  new Promise((resolve, reject) => {
    const data = {
      username,
      surName: lastname,
